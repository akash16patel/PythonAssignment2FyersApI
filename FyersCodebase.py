...
class FyersCodebase:
    def __init__(self,client_id,access_token,logger=None):
        ...
        self.instruemnt_Path="all_instrument.csv"
        self.response=rq.get(self.instrument_url)
        with open(self.instruemnt_Path,'wb') as w:
            w.write(self.response.content)
        self.df=pd.read_csv("all_instrument.csv")
        ...
        self.df.to_csv('all_instrument_with_header.csv',index=False)
        ...
        if call_type.lower() == "quote":
            ...
            if response['code']==200:
               return response['d']['v']
                ...
                return "Please check fyersApi_log file error response"
    ...
    def get_order_details(self):
        ...
        #print(response['orderBook'][length-1])
    ...
    def get_atm(self,ltp,underlying,expiry,script_type):
        ...
        # expiry=self.map_expiry_index(expiry)
        # print(expiry)
    ...
    def get_expiries(self,scripname,exchange,expiry_type):
        ...
        if response['code'] != 200:
            return "please check fyerApilog for details"

        expiry=response['data']
        expiry_dates=expiry['expiryData']
        dates=[item['date'] for item  in expiry_dates ]
        return dates
...
if __name__=="__main__":
    ...
    print(f"otm=={otm}")
    expiry=broker.get_expiries(scripname='SBIN-EQ',exchange='NSE',expiry_type='all')
